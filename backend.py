...
def _build_pipeline() -> Any:
    ...
    # via diffusers' native torchao integration. Weights are stored fp8 but the
    # matmul stays bf16, so bf16 LoRAs still apply via dynamic set_adapters (we
    # never fuse). Text encoder + VAE stay bf16 -> ~34 GB resident, which fits
    # ZeroGPU's `large` (48 GB) tier and ~halves the per-call GPU materialization,
    # so the call completes inside ZeroGPU's proxy-token TTL. (The bf16 58 GB model
    # took ~140 s/call -> exceeded the TTL -> the image was produced server-side
    # but never delivered.) Needs the bumped (latest) diffusers + torchao; the
    # previously-pinned diffusers@973a077 was incompatible with current torchao.
